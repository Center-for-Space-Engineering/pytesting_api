...
logger = loggerCustom("logs/aop1_adc_calibration_test.txt")
...
def setup_before_all_tests():
    ...
    table_structure = {
    'calibration_AOP1_ADC_low' : [['DAC_word', 0, 'string'], ['measured_voltage', 0, 'string'], ['set_current', 0, 'string'], ['measured_current', 0, 'string'], ['gain', 0, 'string'], ['variance_c', 0, 'string'], ['sigma_c', 0, 'string'], ['starting_line', 0, 'string'], ['ending_line', 0, 'string']],
    'calibration_AOP1_ADC_high' : [['DAC_word', 0, 'string'], ['measured_voltage', 0, 'string'], ['set_current', 0, 'string'], ['measured_current', 0, 'string'],['gain', 0, 'string'], ['variance_c', 0, 'string'], ['sigma_c', 0, 'string'], ['starting_line', 0, 'string'], ['ending_line', 0, 'string']],
    }
    ...
    @pytest.mark.AOP1_ADC #MAKE SURE TO ADD THE TEST GROUP (PROCEDURE)
    def test_low_gain():
        try :
            #get system varibles
            ...
            logger.send_log(f"Starting idx: {idx_db_last + 1}")
            ...
            cmin_l = float(global_test_variables.tests_parameters_dict['AOP1_ADC'][0])
            cmax_l = float(global_test_variables.tests_parameters_dict['AOP1_ADC'][1])
            c_step_count = int(global_test_variables.tests_parameters_dict['AOP1_ADC'][4])
            vmin = float(global_test_variables.tests_parameters_dict['AOP1_ADC'][5])
            vmax = float(global_test_variables.tests_parameters_dict['AOP1_ADC'][6])
            v_step_size = float(global_test_variables.tests_parameters_dict['AOP1_ADC'][7])
            ...
            for voltage in setVoltage:
                ...
                requests.get(f'http://{hostname}:{port}/command_AOPDAC_config/AOPDAC_config/{dac_word}/32768/32768/local_code=501', timeout=10)
                requests.get(f'http://{hostname}:{port}/command_AOPDAC_config/AOPGS_config/1/1/local_code=501', timeout=10)
                ...
                for current in setCurrentLow:
                    ...
                    logger.send_log(f"current {current} sent to keithley")
            ...
            global_test_variables.save_data_to_db(table_name='calibration_AOP1_ADC_low', data=data, thread_name='AOP1_calibration')
    ...
    def test_high_gain():
        try :
            ...
            # get parameters out of file
            cmin_h = float(global_test_variables.tests_parameters_dict['AOP1_ADC'][2])
            cmax_h = float(global_test_variables.tests_parameters_dict['AOP1_ADC'][3])
            c_step_count = int(global_test_variables.tests_parameters_dict['AOP1_ADC'][4])
            vmin = float(global_test_variables.tests_parameters_dict['AOP1_ADC'][5])
            vmax = float(global_test_variables.tests_parameters_dict['AOP1_ADC'][6])
            v_step_size = float(global_test_variables.tests_parameters_dict['AOP1_ADC'][7])
            ...
            #set sweep based on parameters 
            setVoltage = np.linspace(vmin,vmax,number_of_steps) # start voltage, max voltage, number of points
            setCurrentLow = np.linspace(cmin_h,cmax_h,c_step_count) # start voltage, max voltage, number of points
            ...
            for voltage in setVoltage:
                ...
                dac_word = SLP_calibrated_dac_code(voltage=voltage) # call calibrated dac word conversion. 
                requests.get(f'http://{hostname}:{port}/command_AOPDAC_config/AOPDAC_config/{dac_word}/32768/32768/local_code=501', timeout=10)
                requests.get(f'http://{hostname}:{port}/command_AOPDAC_config/AOPGS_config/0/1/local_code=501', timeout=10)
                logger.send_log(f"voltage {voltage} dac_word {dac_word}")
                for current in setCurrentLow:
                    # Turn on auto ranging for the keithley
                    instrument_dict['keithley6221'].write(["OUTP OFF"])
                    ...
                    #wait for more data to come in.
                    idx_db = global_test_variables.get_last_idx('AOP')
                    total_liens = granule_count * packet_count
                    while idx_db < (idx_db_last + total_liens): # wait for next idx to come in for board
                        idx_db = global_test_variables.get_last_idx('AOP') # NOTE: We dont need a sleep statement here because the get_last_idx has that already.         
            ...
            global_test_variables.save_data_to_db(table_name='calibration_AOP1_ADC_high', data=data, thread_name='AOP1_calibration')
            logger.send_log("sent save command")
