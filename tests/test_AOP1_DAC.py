...
logger = loggerCustom("logs/AOP1_DAC_Calibration.txt")
...
def setup_before_all_tests():
   ...
   table_structure = {
      f'calibration_AOP1_DAC' : [['DAC_WORD', 0, 'string'], ['V_DAC', 0, 'string']],
   }
   ...
   requests.get(f'http://{hostname}:{port}/command_from_file/send_mode_packet/80/local_code=501', timeout=10) # turn on AOP packets




@pytest.mark.AOP1_DAC #MAKE SURE TO ADD THE TEST GROUP (PROCEDURE)
def test_test():
   #get system varibles
   ...
   vmin = float(global_test_variables.tests_parameters_dict['AOP1_DAC'][0])
   vmax = float(global_test_variables.tests_parameters_dict['AOP1_DAC'][1])
   step_size = float(global_test_variables.tests_parameters_dict['AOP1_DAC'][2])
   settling_time = float(global_test_variables.tests_parameters_dict['AOP1_DAC'][3])
   over_sample_num = int(global_test_variables.tests_parameters_dict['AOP1_DAC'][4])
   ...
   try :
      ...
      global_test_variables.save_data_to_db(table_name='calibration_AOP1_DAC', data=data, thread_name='AOP1_dac_calibration')
