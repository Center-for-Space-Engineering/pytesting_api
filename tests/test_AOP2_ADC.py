...
logger = loggerCustom("logs/aop2_adc_calibration_test.txt")
...
def setup_before_all_tests():
   ...
   table_structure = {
      f'calibration_AOP2_ADC_low' : [['DAC_word', 0, 'string'], ['measured_voltage', 0, 'string'], ['set_current', 0, 'string'], ['measured_current', 0, 'string'], ['gain', 0, 'string'], ['variance_c', 0, 'string'], ['sigma_c', 0, 'string'], ['starting_line', 0, 'string'], ['ending_line', 0, 'string']],
      f'calibration_AOP2_ADC_high' : [['DAC_word', 0, 'string'], ['measured_voltage', 0, 'string'], ['set_current', 0, 'string'], ['measured_current', 0, 'string'],['gain', 0, 'string'], ['variance_c', 0, 'string'], ['sigma_c', 0, 'string'], ['starting_line', 0, 'string'], ['ending_line', 0, 'string']],
   }
...
@pytest.mark.AOP2_ADC #MAKE SURE TO ADD THE TEST GROUP (PROCEDURE)
def test_low_gain():
   try :
      #get system varibles
      ...
      cmin_l = float(global_test_variables.tests_parameters_dict['AOP2_ADC'][0])
      cmax_l = float(global_test_variables.tests_parameters_dict['AOP2_ADC'][1])
      c_step_count = int(global_test_variables.tests_parameters_dict['AOP2_ADC'][4])
      vmin = float(global_test_variables.tests_parameters_dict['AOP2_ADC'][5])
      vmax = float(global_test_variables.tests_parameters_dict['AOP2_ADC'][6])
      v_step_size = float(global_test_variables.tests_parameters_dict['AOP2_ADC'][7])
      ...
      for voltage in setVoltage:
         ...
         requests.get(f'http://{hostname}:{port}/command_AOPDAC_config/AOPDAC_config/32768/{dac_word}/32768/local_code=501', timeout=10)
      ...
      global_test_variables.save_data_to_db(table_name='calibration_AOP2_ADC_low', data=data, thread_name='AOP2_calibration')
...
def test_high_gain():
   try :
      ...
      # get parameters out of file
      cmin_h = float(global_test_variables.tests_parameters_dict['AOP2_ADC'][2])
      cmax_h = float(global_test_variables.tests_parameters_dict['AOP2_ADC'][3])
      c_step_count = int(global_test_variables.tests_parameters_dict['AOP2_ADC'][4])
      vmin = float(global_test_variables.tests_parameters_dict['AOP2_ADC'][5])
      vmax = float(global_test_variables.tests_parameters_dict['AOP2_ADC'][6])
      v_step_size = float(global_test_variables.tests_parameters_dict['AOP2_ADC'][7])
      ...
      for voltage in setVoltage:
         ...
         dac_word = SLP_calibrated_dac_code(voltage=voltage) # call calibrated dac word conversion. 
         requests.get(f'http://{hostname}:{port}/command_AOPDAC_config/AOPDAC_config/32768/{dac_word}/32768/local_code=501', timeout=10)
         requests.get(f'http://{hostname}:{port}/command_AOPDAC_config/AOPGS_config/1/0/local_code=501', timeout=10)
      ...
      global_test_variables.save_data_to_db(table_name='calibration_AOP2_ADC_high', data=data, thread_name='AOP2_calibration')
      logger.send_log("sent save command")
