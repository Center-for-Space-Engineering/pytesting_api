...
logger = loggerCustom("logs/AOP2_DAC_Calibration.txt")
...
def setup_before_all_tests():
   ...
   table_structure = {
      f'calibration_AOP2_DAC' : [['DAC_WORD', 0, 'string'], ['V_DAC', 0, 'string']],
   }
...
@pytest.mark.AOP2_DAC #MAKE SURE TO ADD THE TEST GROUP (PROCEDURE)
def test_test():
   #get system varibles
   ...
   vmin = float(global_test_variables.tests_parameters_dict['AOP2_DAC'][0])
   vmax = float(global_test_variables.tests_parameters_dict['AOP2_DAC'][1])
   step_size = float(global_test_variables.tests_parameters_dict['AOP2_DAC'][2])
   settling_time = float(global_test_variables.tests_parameters_dict['AOP2_DAC'][3])
   over_sample_num = int(global_test_variables.tests_parameters_dict['AOP2_DAC'][4])
   ...
   try :
      ...
      global_test_variables.save_data_to_db(table_name='calibration_AOP2_DAC', data=data, thread_name='AOP2_dac_calibration')
