...
logger = loggerCustom("logs/FBPI_adc_calibration_test.txt")
...
def setup_before_all_tests():
   ...
   table_structure = {
      f'calibration_FBPI_ADC' : [['DAC_word', 0, 'string'], ['measured_voltage', 0, 'string'], ['set_current', 0, 'string'], ['measured_current', 0, 'string'], ['variance_c', 0, 'string'], ['sigma_c', 0, 'string'], ['starting_line', 0, 'string'], ['ending_line', 0, 'string']],
   }
...
@pytest.mark.FBPI_ADC #MAKE SURE TO ADD THE TEST GROUP (PROCEDURE)
def test_FBPI():
   try :
      #get system varibles
      ...
      cmin_l = float(global_test_variables.tests_parameters_dict['FBPI_ADC'][0])
      cmax_l = float(global_test_variables.tests_parameters_dict['FBPI_ADC'][1])
      c_step_count = int(global_test_variables.tests_parameters_dict['FBPI_ADC'][2])
      vmin = float(global_test_variables.tests_parameters_dict['FBPI_ADC'][3])
      vmax = float(global_test_variables.tests_parameters_dict['FBPI_ADC'][4])
      v_step_size = float(global_test_variables.tests_parameters_dict['FBPI_ADC'][5])
      ...
      for voltage in setVoltage:
         ...
         requests.get(f'http://{hostname}:{port}/command_FBPI_config/FBPI_config/{dac_word}/local_code=501', timeout=10)
         ...
         for current in setCurrentLow:
            ...
            idx_db_last = global_test_variables.get_last_idx('FBPI')
            idx_db = idx_db_last

            #send command to turn on fpp4 and slp packets 
            requests.get(f'http://{hostname}:{port}/command_from_file/send_mode_packet/40/local_code=501', timeout=10)

            
            #wait for more data to come in.
            total_liens = granule_count * packet_count
            while idx_db < (idx_db_last + total_liens): # wait for next idx to come in for board
               idx_db = global_test_variables.get_last_idx('FBPI') # NOTE: We dont need a sleep statement here because the get_last_idx has that already.       
            ...
            data_db_AOP = global_test_variables.get_data_from_data_base(table_name = 'FBPI', starting_idx = sample_idx, max_num_lines = total_liens - 1)
      ...
      global_test_variables.save_data_to_db(table_name='calibration_FBPI_ADC', data=data, thread_name='R=FBPI calibration')
