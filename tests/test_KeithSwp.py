...
StartCurrent = '0e-6' #Keithely start current (A)
EndCurrent = '0.08e-6' #Keithely end current (A)
CurrentSteps = '5' #Number of steps to take
StepDelay = '10' #Keithley step hold time (s)
Gain = '1' #Only valid for SLP and AOP channels. 1 is low gain, 0 is high gain
flushTime = 10 #Timer for database flush
######################################################

logger = loggerCustom("logs/KeithSwp.txt")
logger.send_log(f'Running with Start Current: {StartCurrent} and End Current: {EndCurrent}')
...
@pytest.mark.KeithSwp #MAKE SURE TO ADD THE TEST GROUP (PROCEDURE)
def test_keithley_sweeping(): #MAKE SURE TO HAVE test_ in front of this!
   try :
