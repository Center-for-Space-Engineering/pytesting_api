...
StartCurrent = '-350e-6' #Keithely start current (A)
EndCurrent = '350e-6' #Keithely end current (A)
...
logger = loggerCustom("logs/KeithSwpAOP1Low.txt")
...
@pytest.mark.KeithSwpAOP1Low #MAKE SURE TO ADD THE TEST GROUP (PROCEDURE)
def test_keithley_sweeping_AOP1_Low(): #MAKE SURE TO HAVE test_ in front of this!
   try :
      logger.send_log(f'Running AOP1 Low Gain with Start Current: {StartCurrent} and End Current: {EndCurrent}')
