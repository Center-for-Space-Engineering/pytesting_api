...
TableName = 'AOP' #The table we want to pull from for index book-keeping (change to RIC)
...
StartCurrent = '-30e-6' #Keithely start current (A)
EndCurrent = '30e-6' #Keithely end current (A)
CurrentSteps = '5' #Number of steps to take
StepDelay = '20' #Keithley step hold time (s)
Gain = '0' #Only valid for SLP and AOP channels. 1 is low gain, 0 is high gain
flushTime = 10 #Timer for database flush
######################################################

logger = loggerCustom("logs/KeithSwpAOP2High.txt")
...
@pytest.mark.KeithSwpAOP2High #MAKE SURE TO ADD THE TEST GROUP (PROCEDURE)
def test_keithley_sweeping_AOP2_High(): #MAKE SURE TO HAVE test_ in front of this!
   try :
      logger.send_log(f'Running AOP2 High Gain with Start Current: {StartCurrent} and End Current: {EndCurrent}')
