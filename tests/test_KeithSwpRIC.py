...
StartCurrent = '-0.18e-6' #Keithely start current (A)
EndCurrent = '0.18e-6' #Keithely end current (A)
...
logger = loggerCustom("logs/KeithSwpRIC.txt")
...
@pytest.mark.KeithSwpRIC #MAKE SURE TO ADD THE TEST GROUP (PROCEDURE)
def test_keithley_sweeping_RIC(): #MAKE SURE TO HAVE test_ in front of this!
   try :
      logger.send_log(f'Running RIC with Start Current: {StartCurrent} and End Current: {EndCurrent}')
