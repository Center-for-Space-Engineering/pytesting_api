...
logger = loggerCustom("logs/SLP_DAC_Calibration.txt")
...
def setup_before_all_tests():
   ...
   table_structure = {
      f'calibration_SLP_DAC' : [['DAC_WORD', 0, 'string'], ['V_DAC', 0, 'string']],
   }
   ...
   requests.get(f'http://{hostname}:{port}/command_from_file/send_mode_packet/0400/local_code=501', timeout=10)




@pytest.mark.SLP_DAC #MAKE SURE TO ADD THE TEST GROUP (PROCEDURE)
def test_test():
   #get system varibles
   ...
   vmin = float(global_test_variables.tests_parameters_dict['SLP_DAC'][0])
   vmax = float(global_test_variables.tests_parameters_dict['SLP_DAC'][1])
   step_size = float(global_test_variables.tests_parameters_dict['SLP_DAC'][2])
   settling_time = float(global_test_variables.tests_parameters_dict['SLP_DAC'][3])
   over_sample_num = int(global_test_variables.tests_parameters_dict['SLP_DAC'][4])
   ...
   try :
      for voltage in setVoltage:
         ...
         requests.get(f'http://{hostname}:{port}/command_SWP_config/manSLP/255/{dac_word}/{0}/{1}/local_code=501', timeout=10)
      ...
      global_test_variables.save_data_to_db(table_name='calibration_SLP_DAC', data=data, thread_name='slp_dac_calibration')
